...
async function showSharedFolderSettings() {
  const d = el('div', 'dialog', `
    ...
    <div class="sub">Kütüphane bulutta tutuluyor — sen ve karşındaki kişi hiçbir kurulum
      yapmadan aynı şarkıları, çalma listelerini görürsünüz.</div>
    ...
    <div class="btns">
      ...
      <button class="b ok" id="mSave">Kaydet</button>
    </div>`);
  openModal(d);
  ...
  const save = async () => {
    ...
    if (!v) return;
    state.userName = await API.set_user_name(v);
    closeModal(); refreshView(); setStatus('Ad kaydedildi: ' + v);
  };
  d.querySelector('#mSave').onclick = save;
